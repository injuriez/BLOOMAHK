...
SettingsFileName := A_ScriptDir . "\polaris.ini"
...
Gui, Add, Text, x30 y120, UI Navigation Fix:
Gui, Add, Checkbox, x170 y120 vUiNavigationFix, Enable
...
Gui, Add, Text, x380 y120, Fix the UI navigation when using navigation shake
Gui, Add, Text, x380 y200, If you're wondering, this will open the menu to allow for UI navigation shake.
...
Gui, Add, Checkbox, x290 y60 vNegativeControl, Negative Control
...
SaveSettings:
   ...
   ;     SettingsFileName := A_ScriptDir . "\default.ini"
    ...
    IniWrite, %UINavigationFix%, %SettingsFileName%, General, UINavigationFix
    ...
    IniWrite, lNegativeControl, %SettingsFileName%, Minigame, NegativeControl
...
LoadSettings:
	...
	IniRead, lUINavigationFix, %SettingsFileName%, General, UINavigationFix
	...
	GuiControl,, UINavigationFix, %UINavigationFix%
...
if (UINavigationFix == true)
	{
	tooltip, UINavigationFix: true, %TooltipX%, %Tooltip9%, 9
	}
else
	{
	tooltip, UINavigationFix: false, %TooltipX%, %Tooltip9%, 9
	}
...
if (AutoEnableCameraMode == true)
	...
	if !ErrorLevel
		...
		loop, 10
			{
			leftcounter++
			tooltip, Left Count: %leftcounter%/10, %TooltipX%, %Tooltip9%, 9
			send {left}
			tooltip, Action: Press Left, %TooltipX%, %Tooltip8%, 8
			sleep 150
			}
		sleep 50
		loop, 4
			{
			rightcounter++
			tooltip, Right Count: %rightcounter%/4, %TooltipX%, %Tooltip9%, 9
			send {right}
...
tooltip, Current Task: UINavigationFix, %TooltipX%, %Tooltip7%, 7
if (UINavigationFix == true) {
  send {%NavigationKey%}
  send {d}
  send {enter}
...
PixelSearch, foundX, foundY , FishBarLeft, FishBarTop, FishBarRight, FishBarBottom, 0x858687, %FishBarColorTolerance%, Fast RGB
...
if !ErrorLevel
	...
	if !ErrorLevel
		...
		if (ClickX != MemoryX and ClickY != MemoryY)
			...
			tooltip, Bypass Count: %ClickShakeRepeatBypassCounter%/10, %TooltipX%, %Tooltip12%, 12
			if (ClickShakeRepeatBypassCounter >= 9)
				{
				MemoryX := 0
				MemoryY := 0
				}
...
PixelSearch, , , FishBarLeft, FishBarTop, FishBarRight, FishBarBottom, 0x858687, %FishBarColorTolerance%, Fast RGB
if !ErrorLevel
	{
	settimer, NavigationShakeFailsafe, off
	goto BarMinigame
...
if (NegativeControl == true) {
  WhiteBarSize := (0 - Round( (A_ScreenWidth / 247.03) * (InStr(Control, "0.") ? (Control * 100) : Control)) + (A_ScreenWidth / 8.2759), 0 )
} else {
...
else if (Action == 1) ;zleva na pravou
	{
		SideToggle := false
		send {lbutton up}
		if (AnkleBreak == false)
...
else if (Action == 2) ;zprava do leva
	{
		SideToggle := false
		send {lbutton down}
		if (AnkleBreak == true)
...
BarMinigame2: ;detekce
tooltip, , %TooltipX%, %Tooltip19%, 16
sleep 1
PixelSearch, FishX, , FishBarLeft, FishBarTop, FishBarRight, FishBarBottom, 0x29CAF5, %FishBarColorTolerance%, Fast RGB
tooltip, aaaaaaaaaaaaaaaaa: %ErrorLevel%, %TooltipX%, %Tooltip19%, 16
if !ErrorLevel
	{
	...
	if (FishX < MaxLeftBar)
		{
			...
			tooltip, [%Action%]Direction: Max Left, %TooltipX%, %Tooltip10%, 10
			PixelSearch, ArrowX, , FishBarLeft, FishBarTop, FishBarRight, FishBarBottom, 0x858687, %ArrowColorTolerance%, Fast RGB
				if !ErrorLevel
				{	
	...
	else if (FishX > MaxRightBar)
		{
			...
			tooltip, [%Action%]Direction: Max Right, %TooltipX%, %Tooltip10%, 10
			PixelSearch, ArrowX, , FishBarLeft, FishBarTop, FishBarRight, FishBarBottom, 0x858687, %ArrowColorTolerance%, Fast RGB
				if !ErrorLevel
				{	
					tooltip, ->, %ArrowX%, %FishBarTooltipHeight%, 18
	...
	if !ErrorLevel
		{
			...
			{
				Action := 1
				tooltip, [%Action%]Tracking direction: <, %TooltipX%, %Tooltip10%, 10
			...
			{
				Action := 5
				tooltip, [%Action%]Tracking direction: < (Fast), %TooltipX%, %Tooltip10%, 10
				tooltip, <, %BarX%, %FishBarTooltipHeight%, 19
			}
			else if (Direction < -Deadzone2)
			{
			...
			{
				Action := 0
				tooltip, [%Action%]Stabilizing, %TooltipX%, %Tooltip10%, 10
				tooltip, ., %BarX%, %FishBarTooltipHeight%, 19
			}
		}
	else
